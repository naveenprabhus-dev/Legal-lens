...
export const AskDocumentDrawer: React.FC<AskDocumentDrawerProps> = ({
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    try {
      const docContext = document
        ? `Document: ${document.fileName}. Status: ${document.analysisStatus}. Size: ${document.fileSize} bytes. Summary: ${document.summaryPreview || 'Standard commercial contract'}.`
        : 'Active legal workspace context.';

      const result = await requestGroundedQuestion(question.trim(), docContext, document?.fileName);
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs space-y-3">
      ...
      <form onSubmit={handleAsk} className="space-y-2">
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-1.5 top-1/2 -translate-y-1/2 p-1.5 rounded-md bg-indigo-600 text-white disabled:opacity-40 hover:bg-indigo-700 transition-colors"
      ...
      {/* Answer Area */}
      {response && (
        <div className="mt-3 p-3.5 rounded-xl bg-indigo-50/40 border border-indigo-100 text-xs space-y-2.5">
          ...
          {onSaveQuestionToConsultation && (
            <div className="pt-2 border-t border-indigo-100/50 flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-1 text-[11px] text-indigo-700 hover:text-indigo-900 font-medium py-1 px-2.5 rounded-lg hover:bg-indigo-100/50 transition-colors"
              >
                {savedSuccess ? (
                  <>
